...
#define LOG_USE_COLOR
        ...
        Log::Log(const std::string log_file)
        {
            ...
            // std::string log_file_with_timestamp = std::to_string(timestamp) + "-" + log_file;
            std::string log_file_with_timestamp = log_file;
            fp = fopen(log_file_with_timestamp.c_str(), "w+");
            LogInitFile(fp);
        }
    ...
    if (!L.quiet) {
        ...
        void Log::SetLock(lego_loam::common::log_LockFn fn)
        {
            L.lock = fn;
        }
